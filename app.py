...
def user_input_analysis():
    ...
    if request.method == 'POST':
        ...
        if action_node == 'caminho_grafo':
            ...
            is_pf_node = 'is_pf_node' in request.form
            cpf_node = request.form['cpf_node']
            is_cpf_complete_node = 'is_cpf_complete_node' in request.form

            node_pf_pj = 'PF' if is_pf_node else 'PJ'
            node_cpf_cnpj = cpf_node[3:9] if is_cpf_complete_node else cpf_node
            start_node = node_pf_pj + '_' + node_cpf_cnpj + '_' + removerAcentosECaracteresEspeciais(start_node_name)

            if not start_node:
                error_message = "Por favor, forneça um nó de entrada."
            elif start_node not in G:
                error_message = f"Pessoa/Empresa '{start_node}' não foi encontrada no grafo!"
            else:
...
def expand():
    ...
    if clicked_node not in G:
        return jsonify({"error": f"Pessoa/Empresa '{clicked_node}' não foi encontrada no grafo!"}), 400
...
def navigate():
    ...
    if direction == "previous" and current_viz_index > 0:
        current_viz_index -= 1
        return jsonify({"status": "success", "message": "Navigated to previous visualization."})
    elif direction == "next" and current_viz_index < len(visualization_history) - 1:
        current_viz_index += 1
        return jsonify({"status": "success", "message": "Navigated to next visualization."})
    else:
        return jsonify({"status": "error", "message": "Cannot navigate further in this direction."})
...
def enhance_html_with_js(html):
    """Enhance HTML with JavaScript for interactions."""
    js_snippet = """
    <style>
        .button-container {
            position: fixed;
        ...
        function navigate(direction) {
            ...
            .then(data => {
                if (data.status === "success") {
                    ...
                    alert(data.message || "Error navigating.");
                }
            });
        }
    </script>
    <div class="button-container">
        <a href="/download">
            <button class="btn-primary btn-icon">Download Relacionamentos</button>
        </a>
    </div>
    """
...
def shortest_path():
    result_caminho = None
    graph_data = None  # Variable to hold graph data (nodes and edges)
    ...
    if request.method == 'POST':
        ...
        if action == 'caminho':
            ...
            is_pf3 = 'is_pf3' in request.form
            cpf3 = request.form['cpf3']
            is_cpf_complete3 = 'is_cpf_complete3' in request.form

            name4 = request.form['name4']
            is_pf4 = 'is_pf4' in request.form
            cpf4 = request.form['cpf4']
            is_cpf_complete4 = 'is_cpf_complete4' in request.form

            node3_pf_pj = 'PF' if is_pf3 else 'PJ'
            node3_cpf_cnpj = cpf3[3:9] if is_cpf_complete3 else cpf3
            start_node3 = node3_pf_pj + '_' + node3_cpf_cnpj + '_'+ removerAcentosECaracteresEspeciais(name3)

            node4_pf_pj = 'PF' if is_pf4 else 'PJ'
            node4_cpf_cnpj = cpf4[3:9] if is_cpf_complete4 else cpf4
            start_node4 = node4_pf_pj + '_' + node4_cpf_cnpj + '_'+ removerAcentosECaracteresEspeciais(name4)

            avoid_nodes = ['PEP', 'AUX_EMER', 'BOLSA_FAM']

            # Ensure the nodes exist in the graph
            if start_node3 not in G or start_node4 not in G:
                result_caminho = f"Não há conexão entre as entidades: {start_node3} e {start_node4} !"
            else:
                try:
                    # Try to find the shortest path
                    shortest_path_nodes = nx.shortest_path(G, source=start_node3, target=start_node4)
                    depth = find_connection_depth(G, start_node3, start_node4, max_depth=50)
                    result_caminho = depth
                    # result_caminho = f"Caminho mais curto entre os nós {start_node3} e {start_node4}: \n{shortest_path_nodes}"
                    ...
                    for node in subgraph.nodes():
                        ...
                        nodes.append({
                            ...
                            'shape': 'image',  # This is crucial to use the image as node shape
                    ...
                    for u, v in subgraph.edges():
                        # Access the edge's 'tipo' attribute
                        edge_type = subgraph.edges[u, v].get('tipo', 'undefined')  # Default to 'undefined' if not found
                        edges.append({
                            'from': u,
                            'to': v,
                            'label': edge_type,  # Display the relationship on the edge
                            'title': edge_type   # Set as tooltip text
                        })
    ...
    return render_template('shortest_path.html', result_caminho=result_caminho, graph_data=graph_data)
...
# def export_data():
#     conn = sqlite3.connect('mecai.db')
#     df = pd.read_sql_query("SELECT * FROM pep", conn)
#     output = io.BytesIO()
